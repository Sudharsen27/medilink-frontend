...
const groupByDate = (notifications) => {
  ...
  notifications.forEach((n) => {
    ...
    else if (created >= startYesterday) yesterday.push(n);
    else older.push(n);
  });
...
const Notifications = () => {
  ...
  const handleNavigate = (n) => {
    ...
    switch (n.type) {
      case "appointment":
        window.location.href = `/appointments?highlight=${n.related_entity_id}`;
        break;
      case "prescription":
        window.location.href = `/prescriptions?open=${n.related_entity_id}`;
        break;
      default:
        break;
    }
  ...
  const renderGroup = (title, items) =>
    items.length > 0 && (
      <>
        <h3 className="px-4 py-2 font-semibold text-gray-700 dark:text-gray-300">
          {title}
        </h3>
        {items.map((n) => (
          <div
